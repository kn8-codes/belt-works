...
## 2026-08-25 — Claude Code Stripe transport private proof

- Direction/scope: Nate asked to continue through Stripe integration so the course can take payments when ready. GSD split full commerce ambiguity (`0.5925`, blocked) from private 04C-1 transport ambiguity (`0.0750`, cleared). Full spec: `MESH_BOARD/.../04-public-evidence-funnel/04C-STRIPE-COMMERCE-SPEC.md`.
- Isolation/classification: created `/Users/kn8/projects/belt-works-claude-commerce` on `feat/claude-code-course-stripe-readiness`, based on pushed course-page commit `805a971`. `code_lifecycle=maintained_public`; `ponytail_mode=off`; payment, personal-data, entitlement and long-lived public code.
- Official reference: extracted Stripe Checkout quickstart, fulfillment, webhook-signature and CLI documentation to `.firecrawl/commerce-20260825/`. The implemented boundary follows Stripe's requirements that success redirects are not reliable fulfillment, webhooks are required, fulfillment is safe to repeat, and server code retrieves the Checkout Session and expanded line items before deciding status.
- Result: installed official `stripe@22.5.0`; added server-only config/runtime, Checkout action, raw-body webhook signature verification, delayed-payment state handling, Product/Price/quantity/terms/payment checks, success and cancel pages, and data-minimized event/order persistence. Checkout uses one server-owned `STRIPE_COURSE_PRICE_ID` and matching `STRIPE_COURSE_PRODUCT_ID`; no client amount or product selector exists.
- Interlock: defaults are `COURSE_SALES_ENABLED=false` and `COURSE_CHECKOUT_VISIBLE=false`. Complete test configuration requires matching test key, webhook secret, Product ID, Price ID, same-origin base/terms/refund URLs, terms version, access description, a separate 32+ character return-signing secret, `test_only` fulfillment and Supabase server credentials. Live mode always adds `live_fulfillment_adapter_not_installed`, so environment values alone cannot activate live payment.
- Webhook/receipt: accepted types are `checkout.session.completed`, `checkout.session.async_payment_succeeded` and `checkout.session.async_payment_failed`. Signed completed-but-unpaid sessions record `pending_payment` and grant nothing until an asynchronous success event. Paid sessions are re-retrieved from Stripe with expanded line items and upserted by unique Checkout Session/event IDs. Receipt rows omit buyer email and card data; browser roles receive no table access.
- Consent boundary: purchase does not call an update-list provider or create marketing consent. Separate provider packet recommends Buttondown provisionally but creates no account or subscriber: `UPDATE_LIST_PROVIDER_AND_CONSENT_PACKET_V0.md`.
- Dependency security: `stripe@22.5.0` had no advisory. Production audit initially found inherited `ws@8.20.0` through `@supabase/realtime-js`; an explicit package override pinned `ws@8.21.3`. `npm audit --omit=dev` then reported zero vulnerabilities and the commerce/evidence tests, Svelte check, build and scoped ESLint remained green. Full-repo `npm run lint` retains one inherited unrelated failure at `src/routes/problem-map/+page.svelte:284` (`cameraFrame` never assigned); it was reproduced and deliberately not mixed into this branch. The dev-inclusive install audit still reports 8 tooling findings (1 low, 2 moderate, 5 high); no broad Svelte/Vite audit fix was mixed into this payment branch.
- Course-page review remediation: the initial evidence-page review requested changes with 0 critical, 4 important and 1 minor finding. The current successor branch serves seven fictional evidence artifacts under `/course-evidence/claude-code-beyond-chat-window/`; shows and links the literal starter→reviewed unified diff and literal check output; links the task packet, notes, starter, reviewed fixture, check and public-subset README; replaces low-contrast small `--dim` text with `--muted`; labels the unavailable sample `Sample in private production`; states `self-paced` and `English master`; and contains preformatted proof content without document overflow. The six source-derived artifact bytes match the reviewed fictional source, the generated diff matches the page exactly, the README references only present files/working commands, starter check exits expected 1, completed check exits 0, and the course plus seven artifact URLs return HTTP 200. Final focused review passed the literal diff, public README, sample-status and format-fact remediations; its only residuals were the now-corrected full-lint receipt wording and stale staging.
- Independent review remediation: initial read-only commerce review found no critical issues but identified account-wide event contamination, live receipt processing and public session-lookup abuse among its important findings. Remediation now ignores unrelated signed events before provider/database work; blocks all live receipt paths; retrieves and validates Product ID, Price ID, quantity and terms before pending/failed writes; requires a separate 32+ character HMAC return-signing secret; signs a per-Checkout nonce into the return URL and Checkout metadata; and requires strict Session ID syntax plus valid nonce/signature before the public status route may call Stripe. Expanded tests cover these boundaries.
- Automated proof: `npm run test:commerce` passed 26/26 and `npm run test:course-evidence` passed 5/5; covering missing prerequisites, hidden/disabled checkout, test/live key mismatch, same-origin commercial URLs, hard live-mode refusal, Product/Price/quantity/terms/payment validation, data minimization and exact-body synthetic Stripe signatures with tamper rejection. `npm run check` reported 0 errors / 0 warnings; `npm run build`, scoped ESLint and `git diff --check` passed.
- Compiled-server proof with no commerce environment: course route HTTP 200 and contained no `<form>` or test CTA; disabled action returned SvelteKit failure status 503 and created no session; unsigned webhook returned HTTP 400; invented `cs_` return rendered `Payment could not be verified` and `No access has been granted`; cancel route explicitly declined to infer purchase.
- Browser proof: fresh final-build Browser Harness pass covered the course, invented-session return and cancel routes at 1280×900 and 390×844. All 6/6 combinations had one H1, zero heading skips, no horizontal overflow, one inherited `main`, zero default forms/buttons, exact `noindex,nofollow` and the required no-price/no-access/no-purchase marker. One earlier pass was explicitly discarded after a concurrent reviewer build replaced hashed assets under its running preview; process log proved `ENOENT` on the stale hash.
- External effects: no Stripe/Supabase/Buttondown login, credentials, Product/Price, webhook registration, database migration, test/live payment, email collection, merge, deployment, publication or spend.
- Checked by: Egon/default, 2026-08-25.

## 2026-08-25 — Claude Code course evidence-page local proof

**Historical scope note:** this block describes the parent commit `805a971`. Its initial QA was later challenged by independent review. The current successor branch `feat/claude-code-course-stripe-readiness` contains the remediations recorded above.

- Approval/scope: Nate directed the marketing/distribution lane toward a sales page, funnel, keywords and credible web presence, then requested a GSD re-run. Owner decisions: public evidence/prelaunch page; optional course-production plus enrollment-opening updates; fixture/check as primary proof; production receipts and existing work as support; no price before price/refund/access approval. Private implementation only; no public or account effects.
- Isolation: created `/Users/kn8/projects/belt-works-claude-course` on `feat/claude-code-course-sales-page` from `origin/main` `8e6aed2`. The dirty `/Users/kn8/projects/belt-works-ai-consultancy` and curriculum checkout remained untouched.
- Code classification: `code_lifecycle=maintained_public`; `ponytail_mode=off`; public buyer-facing and consent/commerce-adjacent; reused the existing SvelteKit layout, self-hosted IBM Plex Mono design system, accessibility conventions, public proof routes and verified course fixture.
- Result: added `src/routes/courses/claude-code-beyond-chat-window/+page.svelte`. The route renders a truthful `In production` private preview; no price, purchase CTA, fake player, testimonial, scarcity or Anthropic-affiliation claim. It includes the chat-to-project threshold, one-file task packet, reviewed fixture facts/open questions, named check, check limitation, `ACCEPT/REVISE/REJECT`, six-module curriculum, readiness burden, delivered/planned asset ledger, Belt.works proof links, inactive update-consent contract and FAQ.
- Search/credibility source: `/Users/kn8/MESH_BOARD/projects/ai-education-consultancy-2027/claude-code-makerspace/KEYWORD_AND_CREDIBILITY_MAP_V0.md`. Four bounded Firecrawl searches returned eight results each. The map treats snippets as category language, not volume/demand evidence, and selects qualified `Claude Code course for existing AI-chat users` positioning over complete/10×/zero-to-hero claims.
- Technical SEO finding: live `https://www.belt.works/sitemap.xml` returned HTTP 404; source metadata also uses both `belt.works` and `www.belt.works`. Shared sitemap/canonical repair is a separate collision-checked lane. The private route keeps `noindex,nofollow`.
- Build verification: after `npm ci` installed the unchanged lockfile, `npm run check` → 0 errors / 0 warnings; `npm run build` → pass; `npx eslint src/routes/courses/claude-code-beyond-chat-window` → pass; `git diff --check` → pass.
- Dependency note: `npm ci` reported 9 existing vulnerabilities (1 low, 2 moderate, 6 high). No audit fix or dependency mutation was attempted.
- Runtime verification: local production preview `http://127.0.0.1:4192/courses/claude-code-beyond-chat-window` returned HTTP 200 and contained the hero, fixture proof, `noindex,nofollow` and inactive consent markers.
- Browser verification: isolated local Chrome/Browser Harness exercised 1280×900 and 390×844. Both reported `overflow=false`, one H1, 24 headings, zero heading skips, zero forms/fields/buttons, 18 nonempty links, zero broken anchors, zero external resources, six FAQ details, and one each main/nav/footer. Keyboard sequence reached the skip link, shared navigation, both page CTAs, proof links and FAQ summaries. Initial CTA focus inherited a thin blue browser outline; route-local remediation now computes `outline: 2px solid rgb(215,255,99)` with `3px` offset.
- Visual verification: the initial screenshots showed no blocking hierarchy, clipping, spacing, credibility or false-enrollment issue. However, later independent calculation found small `--dim` text below the required contrast ratio; the successor branch changes those uses to `--muted` and verifies the computed color.
- Requirement scan: the initial 15/15 scan covered CCC-014..017 page status, no active form/price, six modules, prerequisite burden, consent purpose/inactive state, title/noindex, fixture/check summary, human decision, existing proof links, false-availability exclusions and no external embeds. It did not prove that the primary fixture/diff/check were independently inspectable; the successor branch adds the public evidence set and visible diff.
- Guardrails at initial verification: no merge, deployment, DNS, Vercel, shared header/navigation, sitemap mutation, provider/account access, email collection, live checkout, analytics, posting, outreach or spending. The parent feature branch was subsequently committed and pushed as `805a971`; `main` and production remained untouched.
- Checked by: Egon/default, 2026-08-25.
